<template>
  <div style="position: relative"><canvas style="width: 100%; display: block"></canvas><div class="annotationLayer"></div><pdf-resize initial @resize="resize"></pdf-resize></div>
</template>
...
<script>
  'use strict'
  import pdfResize from './pdf-resize'
  let PDFJS = null
  ...
  function PDFJSWrapper (PDFJS, canvasElt, annotationLayerElt, emitEvent) {
    ...
    this.renderPage = function (rotate) {
      ...
      let viewport = pdfPage.getViewport(canvasElt.offsetWidth / pdfPage.getViewport(1).width, rotate)
  ...
  export default {
    ...
    methods: {
      resize: function (size) {
        let canvasElt = this.$el.childNodes[0]
        let annotationLayerElt = this.$el.childNodes[1]
        canvasElt.style.height = canvasElt.offsetWidth * (canvasElt.height / canvasElt.width) + 'px'
        let resolutionScale = this.pdf.getResolutionScale()
